 ...
 *   <br>   MAY 02, 2005    (CANAGL) adding command AssignClipboardVariable
 *   <br>   JUL 27, 2005    (CANAGL) adding command CallRemote contributed by Steve Sampson
 *   <br>   AUG 04, 2006    (CANAGL) adding commands AppMapChaining, AppMapResolve, GetAppMapValue
 *   <br>   AUG 22, 2006    (bolawl) updated application() to call Runtime.exec() with null env params for LaunchApplication
 *   <br>   APR 15, 2008    (JunwuMa)added GetCompScreenResolution
 *   <br>   MAY 27, 2008    (JunwuMa)added ClearAllVariables 
 *   <br>   AUG 26, 2008    (CANAGL)Fixed formatted output of GetSystemDate, GetSystemTime, GetSystemDateTime 
 ...
 *   <br>	JUN 04, 2010	(CANAGL) In application(), check Process exitValue to check for success or failure in launching app.
 *   									Reverting to correct use of reverted ProcessConsole.
 *   <br>   NOV 15, 2011    (SBJLWA) Modify method getSystemDateTime(): Add an optional parameter to make it convert to the military time.
 *                                   By default, the method will convert date to AM-PM time as before.
 *   <br>   JAN 27, 2014    (SBJLWA) Modify method clearArrayVariables(): don't clear SAFS-reserved variables.
 *   <br>   APR 29, 2014    (SBJLWA) Modify method getAppMapValue(): use default map if mapid is not provided;
 *                                                                   set map item value to testrecord's statusinfo so jsafs user can get it easily.
 *   <br>   NOV 26, 2014    (SBJLWA) Modify method saveClipboard()/verifyClipboard(): Call deduceXXXFile() to get test/bench file.
 *   <br>   JAN 04, 2015    (SBJLWA) Add method scrollWheel().
 **/
public class DCDriverMiscCommands extends DriverCommand {
   ...
   * <br>   JUL 27, 2005    (CANAGL) adding command CallRemote contributed by Steve Sampson
   * <br>   AUG 03, 2006    (CANAGL) Addition of AppMapResolve and AppMapChaining commands
   **/
  public void process() {
    try {
   ...
   * <br> AUG 22, 2006    (bolawl) updated LaunchApplication to call Runtime.exec() with null env params
   **/
