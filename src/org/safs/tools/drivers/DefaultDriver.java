 ...
 * 
 * <br>(CANAGL) DEC 10, 2008 Fixed LogMode problems affecting XML Logging
 ...
 * <br>	Jul 08, 2011	(CANAGL) 	Add support for PREFERRED_ENGINES_OVERRIDE
 * <br>	SEP 12, 2013	(CANAGL) 	Add Daemon threads for SAFSMonitor shutdown.
 * 
 * 
...
public abstract class DefaultDriver extends AbstractDriver {
	 ...
	 * treated as relative to the "datapool" directory.  Thus, the "datapool" directory must be 
	...
	protected String getProjectDirectoryInfo(String configItem, String subdir){
			...
			if (! fdir.isDirectory()) {
				...
				if (! fdir.isDirectory()){				
					...
					if(fdir.isAbsolute()){
						...
						fdir = new CaseInsensitiveFile(fprj, configPath);
						try{ fdir.mkdirs();}catch(Exception x){
							throw new NullPointerException(x.getClass().getName()+", "+x.getMessage());
						}
						if(! fdir.isDirectory()) throw new NullPointerException(fdir.getAbsolutePath());
	...
	protected void _resetModifiedProperties(){
	 ...
	 * **/
	protected void validateRootConfigureParameters(){
	 ...
	 * We need a test name, test level, and field separator.
	 * If no separator is found we will default to a comma (,) separator.
	 * (CANAGL) DEC 10, 2008 Fixed LogMode problems affecting XML Logging 
	 **/
	 ...
	 * instances.
	 * (CANAGL) 2013.09.23 Watch the order of initialization to be reverse of shutdown. **/
	 ...
	 * (CANAGL) 2013.09.23 Change the order of shutdown to be reverse of initialization. **/	
	...
	public void run(){
		...
		// CANAGL moved here so we don't null too soon and cause NullPointerException at safsmonitor.dispose()
