 ...
 *
 * @author Carl Nagle JAN 27, 2005 Made STAFProc constant compatible with Linux/Unix
 * @author Carl Nagle DEC 14, 2005 Refactored to support STAF shutdown if weStartedSTAF
 * @author Carl Nagle AUG 11, 2008 Doubled the time to wait for STAFServiceLoader to come online.
 ...
 * @author Carl Nagle NOV 13, 2014 Fixed logMessage to try LogUtilities before Driver to get Console logging.
 * @author Lei Wang APR 01, 2019 Added getEmbedded64JVMBinPath(), modified configureJSTAFServiceEmbeddedJVMOption(): use 64-bits embedded Java to run JSAFS Service.
 */
public abstract class DriverConfiguredSTAFInterfaceClass
	...
	public File getEmbeddedJVMBinPath() throws FileNotFoundException{
		...
		throw new FileNotFoundException(pre + " SAFS 32-bits 'jre"+ s +"bin' subdirectory not found.");
	}
	 ...
	 * @throws FileNotFoundException if we could not locate an embedded JVM Path.
	 */
	public File getEmbedded64JVMBinPath() throws FileNotFoundException{
		String pre = "DCSIC.getEmbedded64JVMBinPath ";
		char s = File.separatorChar;
		try{
			String rootdir = driver.getDriverRootDir();
			File jvm = new CaseInsensitiveFile(rootdir, "jre"+ s + "Java64"+ s +"jre"+ s +"bin").toFile();
			if(jvm.isDirectory()) return jvm;
		}catch(NullPointerException x){
			throw new FileNotFoundException(pre + x.getClass().getSimpleName()+" "+ x.getMessage());
		}
		throw new FileNotFoundException(pre + " SAFS 64-bits 'jre"+ s +"bin' subdirectory not found.");
	}
		...
		try{
			if( ! staf.isServiceLibraryRunning(JSTAF)){
				...
				if(STAFInstaller.is32BitsSTAF()){
					mod = "JVM="+getEmbeddedJVMBinPath().getAbsolutePath()+ File.separatorChar + "java";
				}else{
					mod = "JVM="+getEmbedded64JVMBinPath().getAbsolutePath()+ File.separatorChar + "java";
				}
