...
public class STAFProcessContainerDriver extends AbstractDriver {
	 ...
	 * determined relative to the root project directory.  All other subdirectories are 
	...
	protected String getProjectDirectoryInfo(String configItem, String subdir){
		...
		// Datapool is default subdir of project
		if (subdir.equalsIgnoreCase(DriverConstant.DEFAULT_PROJECT_DATAPOOL)){
			fprj = new CaseInsensitiveFile(projectRootDir);
		}
		// other subdirs default to subdirs of datapool
		else{
	    	fprj = new CaseInsensitiveFile(datapoolSource);
	    }

		// often null if defaults are in use.
		if (configItem==null) configItem = fprj.getAbsolutePath() + File.separator + subdir;
		
		fdir = new CaseInsensitiveFile(configItem);
		if (! fdir.isDirectory()) fdir = new CaseInsensitiveFile( fprj, configItem);
	...
	/** 
	 * Verify the validity of minimum REQUIRED parameters.
	 ...
	 * Verify existence of Project directory structure.<br>
