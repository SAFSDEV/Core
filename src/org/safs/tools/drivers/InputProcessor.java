 ...
 * <br>APR 18, 2018 	(Lei Wang) Modified constructor InputProcessor(): Set product's name, version and description.
 * <br>APR 27, 2018 	(Lei Wang) Modified processTest():
 *                                  set variable 'activeTableVar' outside of mainloop.
 *                                  add logTableStart() and logTableEnd(): Log explicit message to tell the start/end of 'Cycle/Suite/TestCase'.
 *                                  log the 'End Table/Cycle/Suite/TestCase' after the 'STATUS_REPORT'.
 */
...
public class InputProcessor extends AbstractInputProcessor {
	/** 'Input Processor'  */
	public static final String PRODUCT_NAME = "Input Processor";
	/** '1.0' */
	public static final String PRODUCT_VERSION = "1.0";
	/** 'The Processor to handle each record from test table.' */
	public static final String PRODUCT_DESCRIPTION = "The Processor to handle each record from test table.";
...
loopbody: {
			...
			// INCREMENT appropriate status/counters for OTHER RECORDS
				...
				//does driver command require branching to BlockID?  (11.15.2005 Bob Lawler RJL)
...
holdloop:	while(! driverStatus.equalsIgnoreCase(JavaHook.RUNNING_EXECUTION)){
		...
		logTableEnd(sourceinfo);

		return statusCounter;
	}

	private void logTableStart(UniqueStringFileInfo sourceinfo){
		String table = sourceinfo.getSourcePath(driver);
		logMessage(sourceinfo.getTestLevel() +" TABLE: "+ table,
		           null, AbstractLogFacility.START_DATATABLE);
		if(isCycle){
			logMessage(table, null, AbstractLogFacility.START_CYCLE);
		}else if(isSuite){
			logMessage(table, null, AbstractLogFacility.START_SUITE);
		}else if(isStep){
			logMessage(table, null, AbstractLogFacility.START_TESTCASE);
		}
	}

	private void logTableEnd(UniqueStringFileInfo sourceinfo){
		String table = sourceinfo.getSourcePath(driver);
		logMessage(sourceinfo.getTestLevel() +" TABLE: "+ table,
		           null, AbstractLogFacility.END_DATATABLE);
		if(isCycle){
			logMessage(table, null, AbstractLogFacility.END_CYCLE);
		}else if(isSuite){
			logMessage(table, null, AbstractLogFacility.END_SUITE);
		}else if(isStep){
			logMessage(table, null, AbstractLogFacility.END_TESTCASE);
		}
	}
