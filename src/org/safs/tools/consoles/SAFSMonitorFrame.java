...
public class SAFSMonitorFrame extends JFrame implements ActionListener, KeyListener
{
	...
	/**
	 * The indicator tells that the current step is from sebuilder's script.
	 */
	private static final String SEBUILDER_STEP = "SEBUILDER_STEP";
	...
	private void showCurTestRecordOnGUI(){
		...
		String testrecord = curtestRecordData.getInputRecord();
		//If the test record is in JSON format, then we consider that it is a sebuilder's step
		if(Utils.isJSON(testrecord)){
			try{
				//the first row is the whole test record
				Vector<Object> newRow = new Vector<Object>(2);
				newRow.add(SEBUILDER_STEP);
				newRow.add(testrecord);
				watchTableModel.addRow(newRow);

				//For sebuilder step, the test record is a JSON string, convert it to a Map object.
				Map<Object, Object> sebuilderStepParams = Utils.fromJsonString(testrecord, Map.class);
				Set<Object> keys = sebuilderStepParams.keySet();
				Object value = null;
				for(Object key:keys){
					newRow = new Vector<Object>(2);
					if(!WDStep.isUnModifiableParameter(key.toString())){
						newRow.add(key);
						value = sebuilderStepParams.get(key);
						if(value instanceof Map){
							value = Utils.toJsonString(value);//convert Map object to JSON string format
						}
						newRow.add(value);
						watchTableModel.addRow(newRow);
					}
	...
	private void updateCurTestRecordFromGUI(){
		...
		if(SEBUILDER_STEP.equalsIgnoreCase(watchTableModel.getValueAt(0, 0).toString())){
			try{
				String currentStep = curtestRecordData.getInputRecord();
				Map<Object, Object> sebuilderStep = Utils.fromJsonString(currentStep, Map.class);
				//The parameters start from the index 1
				for (int i=1; i<watchTableModel.getRowCount(); i++) {
					sebuilderStep.put(watchTableModel.getValueAt(i, 0), watchTableModel.getValueAt(i, 1));
				}

				curtestRecordData.setInputRecord(Utils.toJsonString(sebuilderStep));
			}catch(Exception e){
				processException(e);
			}
			return;
		...
		winitem.add(watchTableModel.getValueAt(1, 0));  // section (String)watchTableModel.getValueAt(1, 0)
		winitem.add(watchTableModel.getValueAt(1, 0));  // item
		winitem.add(watchTableModel.getValueAt(1, 1));	// value  winGuiId

		Vector compitem = new Vector(3);
		compitem.add(watchTableModel.getValueAt(1, 0));  // section (String)watchTableModel.getValueAt(1, 0)
		compitem.add(watchTableModel.getValueAt(2, 0));  // item
		compitem.add(watchTableModel.getValueAt(2, 1));	//  value	compGuiId
