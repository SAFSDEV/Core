 ...
 *   <br>   SEP 10, 2003    (Carl Nagle) Removed WARNING messages for cases "not implemented"
 *   <br>   SEP 16, 2003    (Carl Nagle) Implemented use of new SAFSLOGS logging.
 *   <br>   NOV 04, 2003    (Carl Nagle) Modified .process engine failures from GENERAL_FAILURES to
 *                                   SCRIPT_NOT_EXECUTED so that other engines (like Classic)
 *                                   can attempt to execute the record.
 *   <br>   NOV 05, 2003    (Carl Nagle) .process will now catch and report RuntimeExceptions.
 *   <br>   NOV 10, 2003    (Carl Nagle) Added isSupportedRecordType() implementation.
 *   <br>   NOV 14, 2003    (DBauman/Carl Nagle) Added support for breakpoints.
 ...
 *   <br>   NOV 11, 2004    (Carl Nagle) Removed special handling of ClickLinkBeginning\Containing
 *   <br>   MAR 21, 2005    (Carl Nagle) Allow instantiation of Default Comp Function class when compType=null
 *   <br>   MAR 03, 2009    (Carl Nagle) Issue Failure instead of "Unknown Command" when Comp is not found in waitForObject.
 *   <br>   APR 08, 2010    (Lei Wang)Modify method process(): 1. Treat the mixed mode RS.
 ...
 *   <br>   SEP 26, 2014    (Lei Wang)Rename method statusIsOK() to waitForObjectAndCheck():
 *                                   if object cannot be found and there are more engines to try, the give them a chance.
 *   <br>   AUG 05, 2016    (Lei Wang)Modified waitForObjectAndCheck(): if the waitForObject's rc is SCRIPT_NOT_EXECUTED (4),
 *                                   then set the testrecord's status to SCRIPT_NOT_EXECUTED.
 *   <br>   SEP 27, 2017    (Carl Nagle) Modify TSP to support SAFS REST for ALL frameworks (including SeleniumPlus)
 *   <br>   SEP 28, 2017    (Lei Wang) Modify process(): Delay REST check to WebDriverGUIUtilities.waitForObject(), so it will only affect Selenium-webdriver's engine
 *   <br>   SEP 28, 2017    (Carl Nagle) Removing some of my SEP 27 code that now provides no function.
 **/
 ...
 * @throws SAFSException
   ...
   * <br>Carl Nagle added REST diversion to TIDComponents.
   * <br>Carl Nagle removing code previously delaying REST diversion to TIDComponents.
   **/
  @Override
  public void process() {
    ...
    try {
      ...
      if(isMixed){
	      ...
	      String currentcomprec = "";

	      try{
	    	  currentcomprec = testRecordData.getDDGUtils().staf.getAppMapItem(
	      ...
	      if (currentcomprec == null) currentcomprec = "";
	      ...
	      if (( GenericMasterFunctions.INPUTKEYS_KEYWORD.equalsIgnoreCase(action)||
	    	...
	    	if(!getWinAndCompGUIObjects()){
	    		Log.debug("TestStepProcessor.process(): cannot get window/component object for '"+testRecordData.getWinCompName()+"', stop processing command '"+action+"'!");
	    		return;
	    	}
