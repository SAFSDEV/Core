...
public class Uncheck implements StepType, SRunnerType {
	...
	public boolean run(TestRun ctx) {
		...
		if(checkbox == null) {
			ctx.log().error("Uncheck did not find any matching WebElement.");
			return false;
		}
		try{
			if(checkbox.isSelected()){
				...
				ctx.log().info("Item is not selected. No attempt to Uncheck will be made.");
			}
			return true;
		}catch(Exception x){
			ctx.log().error("Uncheck "+ x.getClass().getSimpleName()+", "+ x.getMessage());
			return false;
		}
	}	
}
