...
/**
 * History:
 * OCT 15, 2014    (Lei Wang) Add field 'target' as the event-receiver.
 * MAR 23, 2015    (Lei Wang) Modify waitForClick(): wait the click event ready if we detect the event-fired.
 * MAR 31, 2015    (Lei Wang) Modify getEventListener(): once received 'mousedown', set the global variable immediately
 *                                  Modify onEventFired(): Add debug information to show where the click happened.
 * JUN 04, 2015    (Lei Wang) Modify run(): if the target webelement is stale, we consider click as success.
 * JUN 25, 2015    (Lei Wang) Modify onEventFired(): ignore event's information to save time.
 * NOV 18, 2015    (Lei Wang) Provide a way to disable the DocumentClickCapture.
 * FEB 22, 2016    (Lei Wang) Modify to avoid setting the click listener the second time if the first listener is consumed.
 * FEB 29, 2016    (Lei Wang) Modify to provide the ability to force receiving click-event by html-document.
 * MAR 14, 2016    (Lei Wang) Log debug message for adding/removing/execution of click-listeners.
 * MAR 29, 2016    (Lei Wang) Refactor to "add listener" and "start polling thread" separately, and detect the JS "Alert"
 * 							 before start "polling thread" to listen.
 * OCT 30, 2019    (Lei Wang) Modified getEventListener(): send the debug message to our log (instead of the browser console).
 */
...
public class DocumentClickCapture implements Runnable{
	 ...
	 * History:
	 * <pre>
	 * Jun 25, 2015 (Lei Wang) Retrieve event's information will spend some time, if the event happens on a remote machine, it will take much longer.
	...
	private String getEventListener(){
	 ...
	 (debug()? "        try{"+
		       //evt.target is original element where the event happens; this is the element who receives this event notification.
	 		   "            debug('DocumentClickCapture.getEventListener():  '+evt.type+' happened on '+ evt.target.tagName +'(id='+evt.target.id+'), it is captured in '+this.tagName+'(id='+this.id+')');"+
	 		   "        }catch(err){}\n" : "") +
			   "        switch(evt.type) {\n"+
			   "            case 'mouseup':\n"+
			   "                break;// ignore \n"+
...
//			   "                try{window."+ EVENT_VIEW +"=evt.view;}catch(err){}\n"+
     ...
     (debug()? "                try{debug('DocumentClickCapture.getEventListener(): ===================================   set window."+listenerID+" to ture   ===========================');}catch(err){}\n" : "")+
			   "                break;// ignore \n"+
			   "            case 'click':\n"+
			   "                break;// ignore \n"+
               "        }\n"+
	 ...
	 */
	private static final String GLOBAL_VAR_BORDER_STYLE = "window.global_safs_var_borderStyle";
