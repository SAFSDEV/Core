 ...
 *  <br>   SEP 06, 2017    (Lei Wang) Added method isType(): provide a robust way to check an object's type.
 *                                  Modified related methods: replace 'obj instanceof Array' by isType(obj, 'Array').
 *  <br>   DEC 29, 2018    (Lei Wang) Added method mouseWheel(): send "MouseWheel Event" to DOM element.
 *
 */
...
//		scriptCommand.append("    alert('event fired.');\n");
	...
	/**
	 * Send MouseWheel event to a DOM element.<br>
	 * See <a href="https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent">MouseEvent</a><br>
	 * See <a href="https://developer.mozilla.org/en-US/docs/Web/API/WheelEvent">WheelEvent</a><br>
	 * <br><b>depending level: 0</b><br>
	 *
	 * @param element (<b>Javascript</b>) Object, the DOM object to be clicked.
	 * @param element (<b>Javascript</b>) deltaX, a double representing the horizontal scroll amount.
	 * @param element (<b>Javascript</b>) deltaY, a double representing the vertical scroll amount.
	 * @param element (<b>Javascript</b>) offsetX, it is used to calculate MouseEvent.clientX, which is the X coordinate of the mouse pointer in local (DOM content) coordinates.
	 * @param element (<b>Javascript</b>) offsetY, it is used to calculate MouseEvent.clientY, which is the Y coordinate of the mouse pointer in local (DOM content) coordinates.
	 */
	public static String mouseWheel(){
		StringBuffer scriptCommand = new StringBuffer();

		scriptCommand.append("function mouseWheel(element, deltaX, deltaY, offsetX, offsetY){\n");
		scriptCommand.append("  try{\n");
		scriptCommand.append("    var box = element.getBoundingClientRect();\n");
		scriptCommand.append("    var clientX = box.left + (offsetX || box.width / 2);\n");
		scriptCommand.append("    var clientY = box.top + (offsetY || box.height / 2);\n");
		scriptCommand.append("    var target = element.ownerDocument.elementFromPoint(clientX, clientY);\n");
		scriptCommand.append("    debug('target is '+target);\n");
		scriptCommand.append("    debug('scroll deltaX:'+deltaX+' deltaY:'+deltaY+', element position ('+clientX+', '+clientY+')');\n");
		if(jsDebugLogEnable){
			scriptCommand.append("    if(target!=null) target.style.border='3px solid red';\n");//highlight the target object
		}
		scriptCommand.append("    for (var e = target; e; e = e.parentElement) {\n");
		scriptCommand.append("      if (e === element) {\n");
		scriptCommand.append("        debug('dispatchEvent to '+target);\n");
		scriptCommand.append("        target.dispatchEvent(new MouseEvent('mouseover', {view: window, bubbles: true, cancelable: true, clientX: clientX, clientY: clientY}));\n");
		scriptCommand.append("        target.dispatchEvent(new MouseEvent('mousemove', {view: window, bubbles: true, cancelable: true, clientX: clientX, clientY: clientY}));\n");
		scriptCommand.append("        target.dispatchEvent(new WheelEvent('mousewheel', {view: window, bubbles: true, cancelable: true, clientX: clientX, clientY: clientY, deltaX: deltaX, deltaY: deltaY}));\n");
		scriptCommand.append("      }\n");
		scriptCommand.append("    }\n");
		scriptCommand.append("  }catch(error){\n");
		scriptCommand.append("    debug(error);\n");
	...
	/**
	 * Get the current frame's id or name or the frame itself.<br>
	 *
	 * <br><b>depending level: 0</b><br>
	 *
	 * @return Object, the current frame's id or name or the frame itself.
	 */
	public static String getCurrentFrame(){
		StringBuffer scriptCommand = new StringBuffer();

		scriptCommand.append("function getCurrentFrame(){\n");
		scriptCommand.append("  try{\n");
		scriptCommand.append("    var frame = window.frameElement;\n");
		scriptCommand.append("    if(frame){\n");
		scriptCommand.append("      if(frame.id){\n");
		scriptCommand.append("        return frame.id;\n");
		scriptCommand.append("      }else if(frame.name){\n");
		scriptCommand.append("        return frame.name;\n");
		scriptCommand.append("      }else{\n");
		scriptCommand.append("        return frame;\n");
		scriptCommand.append("      }\n");
		scriptCommand.append("    }else{\n");
		scriptCommand.append("      return null;\n");
		scriptCommand.append("    }\n");
		scriptCommand.append("  }catch(error){\n");
		scriptCommand.append("    debug('JSF.getCurrentFrame(): '+error);\n");
		scriptCommand.append("  }\n");
		scriptCommand.append("}\n");
	...
	/**
	 * Test if an object of a certain type.<br>
	 *
	 * <br><b>depending on: nothing </b><br>
	 * <br><b>depending level: 0</b><br>
	 *
	 * @param object (<b>Javascript</b>) Object, the object to test
	 * @param type (<b>Javascript</b>) String, the type string
	 */
	public static String isType(){
		StringBuffer scriptCommand = new StringBuffer();

		scriptCommand.append("function isType(object, type){\n");
		scriptCommand.append("  try{\n");
		scriptCommand.append("    debug('JSF.isType(): Is '+type+'?: '+object);\n");
		scriptCommand.append("    if(object==undefined){\n");
		scriptCommand.append("      throw new Error('object is not defined!');\n");
		scriptCommand.append("    }else{\n");
		scriptCommand.append("      if(type.toLowerCase()==='"+TYPE_ARRAY+"'.toLowerCase()){\n");
		scriptCommand.append("        return ((object instanceof Array) || (Object.prototype.toString.call(object)==='[object Array]'));\n");
		scriptCommand.append("      }else{\n");
		scriptCommand.append("        throw new Error('Need more implementation for type: '+type);\n");
		scriptCommand.append("      }\n");
		scriptCommand.append("    }\n");
		scriptCommand.append("  }catch(error){\n");
		scriptCommand.append("    debug('JSF.isType(): '+error);\n");
		scriptCommand.append("  }\n");
		scriptCommand.append("}\n");
	...
	public static final class SAP{
		 ...
		 * the AccessibilityController, if any..<br>
		 *
		 * <br><b>depending on: existing DOM and SAP APIs only.</b><br>
		 * <br><b>depending level: 0</b><br>
		 *
		 * @param domElement (<b>Javascript</b>) Object (WebElement), the dom object of the SAP object.
		 */
		public static String sap_getAccessibilityControllerElements(boolean includeDependency){
			StringBuffer scriptCommand = new StringBuffer();
			...
			scriptCommand.append("function sap_getAccessibilityControllerElements(domElement){\n");
			scriptCommand.append("  try{\n");
			scriptCommand.append("    var control = sap_getObject(domElement);\n");
			scriptCommand.append("    if(!control){\n");
			scriptCommand.append("      debug(\"Did not find ANY SAP Object from domElement...aborting.\");\n");
			scriptCommand.append("      return;\n");
			scriptCommand.append("    }\n");
			//TODO what is this getAccessibilityController(), it is not a SAP standard API, is it SAS special? should we test the class instance?
			//objectIsInstanceof()
			//scriptCommand.append("    if(control instanceof some.SAS.special.control) {\n");
			scriptCommand.append("    const accesscontroller = control.getAccessibilityController();\n");
			scriptCommand.append("    if(!accesscontroller){\n");
			scriptCommand.append("      debug(\"Did not find ANY accessibilityController()...aborting.\");\n");
			scriptCommand.append("      return;\n");
			scriptCommand.append("    }\n");
			//TODO what is this getAccessibilityElements(), it is not a SAP standard API, is it SAS special? should we test the class instance?
			scriptCommand.append("    return accesscontroller.getAccessibilityElements();\n");
			scriptCommand.append("  }catch(error){\n");
			scriptCommand.append("    //alert(error);\n");
			scriptCommand.append("    debug(error);\n");
			scriptCommand.append("  }\n");
		...
		/**
		 * According to a dom object, find SAP Accessibility Objects associated with
		 * the AccessibilityController, if any, and return the critical info fields of these.<br>
		 *
		 * <br><b>depending on:</b><br>
		 * {@link #sap_getAccessibilityControllerElements()} <br>
		 * <br><b>depending level: 1</b><br>
		 *
		 * @param domElement (<b>Javascript</b>) Object (WebElement), the dom object of the SAP object.
		 * @param includeDependency true if we need to also prepend the getAccessibilityControllerElements JS code.
		 * @return accessibilities (<b>Javascript</b>) Array, an array of accessibility object containing 'accessibilityFrame',
		 *                                                    'accessibilityHint', 'accessibilityLabel', 'terseAccessibilityLabel', 'isSelected' etc.
		 * @see #sap_getAccessibilityControllerElements()
		 */
		public static String sap_getAccessibilityControllerElementsInfo(boolean includeDependency){
			StringBuffer scriptCommand = new StringBuffer();

			if(includeDependency){
				scriptCommand.append(sap_getAccessibilityControllerElements(includeDependency));
			}

			scriptCommand.append("function sap_getAccessibilityControllerElementsInfo(domElement){\n");
			scriptCommand.append("  try{\n");
			scriptCommand.append("    var accesselements = sap_getAccessibilityControllerElements(domElement);\n");
			scriptCommand.append("    if(!accesselements){\n");
			scriptCommand.append("      debug(\"Did not receive ANY accessibilityElements...aborting.\");\n");
			scriptCommand.append("      return;\n");
			scriptCommand.append("    }\n");
			scriptCommand.append("    var props = new Array();\n");
			scriptCommand.append("    for(var i=0;i<accesselements.length;i++){\n");
			scriptCommand.append("      info = new Object();\n");
			scriptCommand.append("      info."+Accessibility.PROPERTY_ACCESSIBILITY_FRAME+"=accesselements[i].getAccessibilityFrame();\n");
			scriptCommand.append("      info."+Accessibility.PROPERTY_ACCESSIBILITY_HINT+"=accesselements[i].getAccessibilityHint();\n");
			scriptCommand.append("      info."+Accessibility.PROPERTY_ACCESSIBILITY_LABEL+"=accesselements[i].getAccessibilityLabel();\n");
			scriptCommand.append("      info."+Accessibility.PROPERTY_TERSE_ACCESSIBILITY_LABEL+"=accesselements[i].getTerseAccessibilityLabel();\n");
			scriptCommand.append("      info."+Accessibility.PROPERTY_SELECTED+"=accesselements[i].isSelected();\n");
			scriptCommand.append("      props.push(info);\n");
			scriptCommand.append("    }\n");
			scriptCommand.append("    return props;\n");
			scriptCommand.append("  }catch(error){\n");
			scriptCommand.append("    //alert(error);\n");
			scriptCommand.append("    debug(error);\n");
			scriptCommand.append("  }\n");
		 ...
		 * {@link #objectIsInstanceof()} <br>
		 * {@link JavaScriptFunctions#isType()}<br>
		 * <br><b>depending level: 1</b><br>
		 *
		 * @param domelement (<b>Javascript</b>) Object, Dom Object is used to find a SAP object.
		 */
		...
		public static String sap_ComboBox_getItems(boolean includeDependency){
			...
			if(includeDependency){
				scriptCommand.append(sap_getObject());
				scriptCommand.append(parse_sap_ui_core_Item(false));
				scriptCommand.append(objectIsInstanceof());
				scriptCommand.append(isType());
			}
			...
			scriptCommand.append("        if(isType(items, '"+TYPE_ARRAY+"')){\n");
			scriptCommand.append("          for(var i=0;i<items.length;i++){;\n");
		 ...
		 * <br><b>depending on:</b><br>
		 * {@link JavaScriptFunctions#throw_error()} <br>
		 * {@link JavaScriptFunctions#isType()}
		 * {@link #sap_getObject()} <br>
		 * {@link #parse_sap_ui_core_Item()} <br>
		 * <br><b>depending level: 1</b><br>
		 *
		...
		public static String sap_ui_commons_ListBox_getItems(boolean includeDependency){
			...
			scriptCommand.append("        //if it contains more properties, we need to modify parse_sap_ui_core_Item() to get them.\n");
			scriptCommand.append("        if(isType(items, '"+TYPE_ARRAY+"')){\n");
			scriptCommand.append("          debug('items is treated as Array: '+items);\n");
			scriptCommand.append("          for(var i=0;i<items.length;i++){;\n");
			scriptCommand.append("            option = parse_sap_ui_core_Item(items[i], i, selectedKeys);\n");
			scriptCommand.append("            properties.push(option);\n");
			scriptCommand.append("          }\n");
			scriptCommand.append("        }else{\n");
			scriptCommand.append("          debug('items is treated as a single item: '+items);\n");
			scriptCommand.append("          option = parse_sap_ui_core_Item(items, 0, selectedKeys);\n");
			scriptCommand.append("          properties.push(option);\n");
			scriptCommand.append("        }\n");
			scriptCommand.append("      }\n");
			scriptCommand.append("    }\n");
			scriptCommand.append("    debug('properties: '+properties);\n");
			scriptCommand.append("    return properties;\n");
			scriptCommand.append("  }catch(error){\n");
			scriptCommand.append("    //alert(error);\n");
			scriptCommand.append("    throw_error(error);\n");
...
//			scriptCommand.append("      item."+MenuItem.PROPERTY_VISIBLE+"=(jQuery.sap.byId(itemobject.getId())[0].style.visibility==='visible');\n");
		...
		public static String sap_ui_commons_Tree_getNodes_Rec(boolean includeDependency){
			...
			scriptCommand.append("        if(isType(nodes, '"+TYPE_ARRAY+"')){\n");
		 ...
		 */
		static String parse_sap_ui_core_Item(boolean includeDependency){
			StringBuffer scriptCommand = new StringBuffer();
			...
			scriptCommand.append("  try{\n");
			scriptCommand.append("    debug('index '+index+': '+itemobject);\n");
			scriptCommand.append("    if( itemobject!=undefined){\n");
			scriptCommand.append("      var item = new Object();\n");
			...
			scriptCommand.append("      if(isType(selectedKeyArray, '"+TYPE_ARRAY+"')){\n");
...
//			scriptCommand.append("      alert(properties);\n");
		...
		static String parse_dijit_form_AutoCompleterMixin_Item(boolean includeDependency){
			...
			scriptCommand.append("      item."+Item.PROPERTY_SELECTED+"=false;\n");
			scriptCommand.append("      if(isType(selectedItemArray, '"+TYPE_ARRAY+"')){\n");
	...
	public static String getAllFunctions(){
		...
		definition.append(getCurrentFrame());
		...
		definition.append(mouseWheel());

		definition.append(sendHttpRequest(null));
		definition.append(isType());
...
//		definition.append(getJSDebugArray());
	...
	public static String getAllSAPFunctions(){
		...
		definition.append(SAP.sap_getAccessibilityControllerElements(false));
		definition.append(SAP.sap_getAccessibilityControllerElementsInfo(false));

		definition.append(SAP.parse_sap_ui_commons_Tab());
		definition.append(SAP.parse_sap_ui_core_Item(false));
