 ...
 * <br> JUN 26, 2003    (CANAGL)  Added getInitializedHelper and some constants.
 * <br> JUN 30, 2003    (BAUMAN)  Separated out Singleton code to SingletonSTAFHelper.
 * <br> JUN 30, 2003    (CANAGL)  Added additional convenience constructor and made the 
 ...
 * <br> SEP 12, 2003    (CANAGL)  Added SAFSLOGS service data.
 * <br> NOV 09, 2003    (CANAGL)  Adding support for multiple different engine hooks.
 * <br> NOV 10, 2003    (CANAGL)  Capture process/hook name used to create an instance.
 * <br> APR 06, 2004    (CANAGL)  Corrected problems with SAFSLOGS service not being recognized.
 * <br> JUN 07, 2004    (CANAGL)  Added isServiceAvailable to compliment isToolAvailable.
 * <br> JUN 17, 2004    (CANAGL)  Added SAFSINPUT as a predefined tool.
 * <br> JUN 28, 2004    (CANAGL)  Added postNextTestEvent to compliment getNextTestEvent.
 * <br>                           Added set\getSAFSTestRecordData.
 * <br> OCT 31, 2004    (CANAGL)  Catch infinite loop in sendQueueMessage.
 * <br> DEC 07, 2004    (CANAGL)  Fixed setHookTestResult and added Results event timeout.
 * <br> APR 22, 2005    (CANAGL)  Enhanced ordering of initialize.
 * <br> MAY 11, 2005    (CANAGL)  Added missing SEM Event "Shutdown" in resetHookEvents.
 * <br> JUL 25, 2005    (CANAGL)  Changed Results event timeout from 3 seconds to 12 seconds.
 * <br> JUL 27, 2005    (CANAGL)  Added submit2WithVar for STAF submits writing results to SAFSVARS.
 * <br> NOV 15, 2005    (bolawl)  Added support for new TRD statusinfo field (RJL).
 * <br> AUG 04, 2006    (canagl)  setVariable and getAppMapItem modified to accept legal null or empty values.
 ...
 * <br> JUN 15, 2009    (CANAGL)  Fixed QUEUE message service STAFExceptions with STAF V3. 
 * <br> JUN 16, 2009    (LeiWang) Modify method sendQueueMessage(): If STAFException occur, try again.
 * <br> JUL 28, 2009    (CANAGL)  Added some waits\checks for shutdown operations. 
 * <br> SEP 11, 2009    (CANAGL)  Resolving some synch issues between driver and engines. 
 * <br> AUG 23, 2010    (CANAGL)  Add waitForWaiters to fix Driver\Engine EVENT synchronization. 
 * <br> APR 01, 2011 	(Dharmesh4) Modify JSTAF add service options para.
 * <br> JUL 01, 2011    (CANAGL)  Adding generic SAFSLOGS logMessage support.
 * <br> MAY 03, 2012    (SBJLWA)  Modify to adjust the backward compatibility of STAF.
 *                                Add method to check the condition of resetting "shutdown" semaphore.
 * <br> JUL 19, 2012    (CANAGL)  Adding more public constants.
...
//   val = val.trim();
  ...
  public boolean sendQueueMessage (String name, String message) {
    ...
    //CANAGL QUEUE service can use the same length tagged format for both STAF V2 and V3.    
  ...
  public void pulseEvent (String event) throws SAFSException {
    ...
    //CANAGL STAF V3 order of keywords IS important!
  ...
  public void resetEvent (String event) throws SAFSException {
    ...
    //CANAGL STAF V3 order of keywords IS required!
  ...
  public void waitEvent (String event, long delaySeconds) throws SAFSException {
    ...
    if (delaySeconds >= 0) {
    	...
    	//CANAGL the TIMEOUT keyword is new and required for STAF V3
   ...
   *<br>      APR 04, 2004    (CANAGL) Enabled matching on substring of tool name.
   *                                   and made the function public.
   *<br>      APR 06, 2004    (CANAGL) Ignore Case on toolname matches.
   */
  public boolean isToolAvailable(String toolname) {
	  if(EmbeddedHandles.isToolRunning(toolname)) return true;
    String result;
    String service = "HANDLE";
    //(CANAGL) the below change is valid.  STAF V3 does not have QUERY ALL listed as supported.
   ...
   *<br> History:
   *<br>
   *<br>      OCT 09, 2013    (CANAGL) Original Release
   */
   ...
   *<br> History:
   *<br>
   *<br>      JUN 07, 2004    (CANAGL) Original Release
   */
   ...
   **<br>JUN 26, 2003    dbauman: ported to java
   **<br>JUN 07, 2004    CANAGL: mod to use isServiceAvailable
   **/
  ...
  public void waitSTAFMutex (String mutex, long delaySeconds) throws SAFSException {
    ...
    if (delaySeconds >= 0) {
    	String secString = Long.toString(delaySeconds*1000);
    	//CANAGL the TIMEOUT keyword is new for STAF V3
    	command = (STAFVersion < 3) ? command + secString : command + " TIMEOUT "  + secString;  
    }
