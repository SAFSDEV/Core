...
**/
/**
 * <br>	Dec 23, 2008	(Lei Wang)	Add method saveImageToFile(): If the file format is JPG, use the compression quality to save file.
 ...
 * <br> Oct 29, 2009	(Carl Nagle)	Added BitTolerance support.
 * <br> Nov 03, 2009	(Carl Nagle)	Draft Prep for multi-threaded algorithm.
 * <br> Nov 19, 2009	(Carl Nagle)	Removing default of multi-threading BitTolerance searches.
 ...
 * <br>	Apr 07, 2010	(Lei Wang)   Add "ImageRect=" or "ImageRectangle=" mode, which is used to define a top Window.
 ...
 * <br>	Apr 20, 2010	(Lei Wang)   Modify method findComponentRectangle():use static method of OCREngine to get
 *                                  an OCR engine to use.
 * <br>	May 26, 2010	(JunwuMa) 	Updates to make ImageText= work with Index= and SearchRect=
 * <br>	Jul 13, 2010	(Lei Wang) 	Add method findImageWithXYBTThread(),findSmallImageWithXYBTThread(),findBigImageWithXYBTThread()
 ...
 * <br>	Oct 26, 2010	(Carl Nagle) 	Large refactoring to fix implementation for SearchRect and new usePerImageModifiers feature.
 * <br>	Oct 28, 2010	(Carl Nagle) 	Added support for UsePerImageModifiers on Index= and BitTolerance=
 * <br>	DEC 22, 2010	(Lei Wang) 	Added methods getCopiedImage(), paintOnImage() and getImageFileFilter() etc.
 * <br>	DEC 27, 2010	(Lei Wang) 	Added methods paintOnImage(): add one boolean parameter to decide if paint on the original image.
 *                                  Add some image filter operations like blur, sharpen, edge, etc.
 * <br>	AUG 25, 2014	(Lei Wang) 	Fix problem of saving image as .bmp and .jpg file.
 * <br>	SEP 30, 2014	(Lei Wang) 	Add method copy().
 * <br>	SEP 30, 2014	(Lei Wang) 	Modify method getSubAreaRectangle(): move common code to calculateAbsoluteCoordinate().
 ...
 * <br>	DEC 12, 2014	(Lei Wang) 	Move filterImage (cover certain area with black) functinality from DCDriverFileCommands to here.
 * <br> FEB 18, 2016    (Carl Nagle)    Support Comp search of whole screen when WinRec is ImageRect or SearchRect.
 * <br> JUN 26, 2018    (Lei Wang)   Added method convertPDF(): use ghost4j and ghostscript to convert PDF to images.
 *                                  Overloaded method createDiffImage(): add parameter to tint or shade the pixel of no difference when creating diff image.
 */
...
public class ImageUtils {
     ...
     * @author Bob Lawler				-Added 10.21.2005 (RJL)
     **/
	...
	public static void recaptureScreen()throws AWTException
	{
		...
		while(screenImage==null && retry++ < 10){
			try{
				...
				//Lei Wang do we need a copy having the Raster of same size?
...
//				}
	...
	/**
	 * The style to alter the image.<br>
	 * <ul>
	 * <li><b>SHADE</b> Get the image darker
	 * <li><b>TINT</b> Get the image lighter
	 * <li><b>NONE</b> Keep the image unchanged
	 * </ul>
	 */
	public enum AlterImageStyle{
		SHADE("SHADE"),
		TINT("TINT"),
		NONE("NONE");

		public final String name;
		AlterImageStyle(String name){
			this.name = name;
		}

		public static AlterImageStyle get(String name) throws IllegalArgumentException{
			if(SHADE.name.equalsIgnoreCase(name)){
				return SHADE;
			}else if(TINT.name.equalsIgnoreCase(name)){
				return TINT;
			}else if(NONE.name.equalsIgnoreCase(name)){
				return NONE;
			}else{
				throw new IllegalArgumentException(name+" is not a valid AlterImageStyle. '"+SHADE.name+"' '"+TINT.name+"' or '"+NONE.name+"' are permitted.");
			}
		}
	}
	 ...
	 * @param benchimg BufferedImage,
	 * @param style AlterImageStyle, {@link AlterImageStyle} how to alter the pixel of no difference.<br>
	 *                                 Normally, the different pixel will be filled with red color, while the same pixel will be written as it is.<br>
	 *                                 But if the original pixel is red and there is no difference, it will be hard to know if there is a difference at that pixel!<br>
	 *                                 So user may want to get the original pixel (with no difference) lighter or darker.<br>
	 *
	 * @param factor double, how much lighter or darker to make the original pixel (with no difference), it is a number between 0 and 1.<br>
	 *                       It only takes effect if the parameter 'style' is {@link AlterImageStyle#TINT} or {@link AlterImageStyle#SHADE}.<br>
	 *                       For SHADE, the bigger the factor, the darker the shade.<br>
	 *                       For TINT, the bigger the factor, the lighter the tint.<br>
	 * @return BufferedImage highlighting the differences between the two images.
	 * @throws IllegalArgumentException if either of the images provided is null.
	 */
	public static BufferedImage createDiffImage(BufferedImage buffimg, BufferedImage benchimg, AlterImageStyle style, double factor) throws IllegalArgumentException{
	    BufferedImage diffimg = null;

	    if(buffimg==null){
	    	throw new IllegalArgumentException("The parameter buffimg is null.");
	    }
	    if(benchimg==null){
	    	throw new IllegalArgumentException("The parameter benchimg is null.");
	    }

    	if(AlterImageStyle.SHADE.equals(style) ||
    	   AlterImageStyle.TINT.equals(style)){
    		if(factor<0 || factor>1){
    			throw new IllegalArgumentException("For style "+style+", the provided factor '"+factor+"' is not valid, it should be in range [0, 1]");
    		}
    	}
        ...
        for (int x = 0; x < maxwidth; x++) {
            for (int y = 0; y < maxheight; y++) {
                ...
                if((diffr + diffg + diffb) == 0){
                	...
                	}else if(AlterImageStyle.TINT.equals(style)){
                		r1 = (int) (r1 + (255 - r1) * factor);
                		g1 = (int) (g1 + (255 - g1) * factor);
                		b1 = (int) (b1 + (255 - b1) * factor);

                	}else{
                		//no change
                	}

                	diffimg.setRGB(x, y, (int) ((r1*0x010000)+ (g1*0x0100)+ b1));
                }else{
                	//if not matched, set red color
	...
	public static BufferedImage createDiffImage(BufferedImage buffimg, BufferedImage benchimg) throws IllegalArgumentException{
		return createDiffImage(buffimg, buffimg, AlterImageStyle.NONE, 1);
	}

	/**
	 * @param pdfFile File, the PDF file to convert
	 * @param resolution int, the resolution used to convert PDF to image.<br>
	 * @return List&lt;BufferedImage&gt;, a list of BufferedImage converted from PDF file
	 * @throws SAFSException if there is any problem during the conversion.
	 */
	public static List<BufferedImage> convertPDF(File pdfFile, int resolution/*DPI*/) throws SAFSException{

//		System.setProperty("jna.library.path", "C:\\Program Files (x86)\\gs\\gs9.23\\bin");
		...
		try {
			...
			for(Image image:images){
				bufferedImages.add(toBufferedImage(image));
			}
		} catch (Exception e) {
			IndependantLog.error(StringUtils.debugmsg(false), e);
			throw new SAFSException("Failed to convert PDF file '"+pdfFile+"' to images.", e);
		}
	...
	public static BufferedImage toBufferedImage(Image image){
	    ...
	    //Create a empty buffered image
	    BufferedImage bufferedImage = getVoidBufferImage(image.getWidth(null), image.getHeight(null), BufferedImage.TYPE_INT_RGB, BufferedImage.TYPE_INT_ARGB);
	...
	/**
	 * Take screen shot and save it to a file.
	 * @param screenShotFile String, the full file name to hold the screen shot.
	 */
	public static void takeScreenshot(String screenShotFile){
		try {
			BufferedImage image = Robot.getRobot().createScreenCapture(new Rectangle(Toolkit.getDefaultToolkit().getScreenSize()));
			saveImageToFile(image, new File(screenShotFile));
		} catch (Exception e) {
			IndependantLog.error("ImageUtls.takeScreenshot(): Met "+e.toString());
		}
	}
...
//			boolean equal = ImageUtils.compareImage(bf1, bf2, 80);
	...
	private static void testConvertPDF(String...args){
		try {
			if(args.length<1){
				System.out.println("Usage: java ImageUtils PDFFile [folderToHoldImages]");
				return;
			}
			File pdfFile = new File(args[0]);
			File folderToHoldImages = null;
			if(args.length>1){
				folderToHoldImages = new File(args[1]);
			}

			List<BufferedImage> images = ImageUtils.convertPDF(pdfFile, 300);
			for(int i=0; i<images.size(); i++){
				ImageUtils.saveImageToFile(images.get(i), new File(folderToHoldImages, "pdf_"+i+".png"));
			}
