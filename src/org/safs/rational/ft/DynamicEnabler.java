 ...
 * <p>   
 * @author CANAGL  SEP 02, 2005
 * @author CANAGL  NOV 12, 2007 Added support for custom DAT files in Class & JAR directory.
 * @author WangLei NOV 07, 2008 Added .net related field and methods enableNetWindows()
 * @author CANAGL  APR 03, 2009 Refactored primarily removing need for ClassMap.dat files on Windows.
 *                              Also added JNA usage to help find ALL processes that can be enabled.
 * @author CANAGL  Jun 04, 2009 Added getJNAProtectedDomainTopObjects 
 * @author CANAGL  Aug 07, 2009 Added Process= support to enable only one or more specific processes. 
 * @author CANAGL  Apr 16, 2010 Handle NullPointerExceptions in loadWinEnabled when RFT starts to act up. 
...
public class DynamicEnabler {
	...
	protected static void loadWinEnabled(){
		...
		//CANAGL 2009.08.25 believing this is causing instability
		//winenabled.clear();
		...
		for(int i=0;i< domains.length;i++){
			...
			for(int j=0;j<winobjects.length;j++) {
				...
				//CANAGL: new incidents of itestobject being null throwing NPE:
