 ...
 *   <br>   SEP 02, 2005    (bolawl) reverting to previous code w/ some
 *                            cosmetic cleanup.  Also commented out call to
 *                            getGuiImage() as it is clearly not working (RJl).
 *   <br>   OCT 17, 2005    (CANAGL) Added DoubleClick Support
 *   <br>   OCT 28, 2005    (CANAGL) Refactored for click variant extensions
 *   <br>   OCT 31, 2005    (CANAGL) Adding a generic GetGUIIMage submitted by
 ...
 *   <br>   MAY 01, 2009    (CANAGL) Adding Float value support for numeric parameters.
 ...
 *   <br>   JUN 30, 2009    (singrk) Added LeftDrag variations using MouseModifiers
 ...
 *   <br>   NOV 12, 2009    (CANAGL) Attempt to catch Exceptions resulting from intended Window closures.
 *   <br>   FEB 05, 2010    (CANAGL) Adding SendEvent support for Flex. FlexObjectTestObject.performAction() is 
 ...
 *   <br>   SEP 07, 2015    (SBJLWA) Correct a typo, change method preformDrag to performDrag. Modify comments: developer history will not show in java doc.
 **/
...
public class CFComponent extends ComponentFunction {
   ...
   * @exception SAFSException
   * @author CANAGL trying to intercept TargetGoneException when selection intends to close window
   */
  ...
  protected void performDrag() throws SAFSException{
      ...
      try { 
    	  //CANAGL replaced case-sensitive test of "Coords="
  ...
  protected void performClick(Object point){
    ...
    try{
	    ...
	    // CANAGL this probably needs to be changed to logging a full-fledged failure! 
	  	//testRecordData.setStatusCode(StatusCodes.SCRIPT_NOT_EXECUTED);
...
//	}
  ...
  protected void verifyPropertyContains () throws SAFSException {
    ...
    if (params.size() < 2) {
      ...
      // CANAGL -- However, we should probably issue FAILURE and not NOT_EXECUTED
      if (rval == null) {
	      testRecordData.setStatusCode(StatusCodes.SCRIPT_NOT_EXECUTED);
          return;
      }
  ...
  protected void verifyValueContains () throws SAFSException {
    ...
    if (params.size() < 2) {
      ...
      String val =  (String) iterator.next();
      //str = substituteVariable(str); // CANAGL: this has already occurred
      if (str==null) return;
      //val = substituteVariable(val); // CANAGL: this has already occurred
      if (val==null) return;
  ...
  protected void verifyValues () throws SAFSException {
    ...
    if (params.size() < 2) {
      ...
      String str = (String) iterator.next();
      String val =  (String) iterator.next();
      //str = substituteVariable(str); //CANAGL: substitution has already occurred
      if (str==null) return;
      //val = substituteVariable(val); //CANAGL: substitution has already occurred
      if (val==null) return;
   ...
   * @author CANAGL Trying to intercept and ignore TargetGoneException when input may intend to close window.
   **/
  protected void inputKeystrokes () throws SAFSException {
    ...
    if (params.size() < 1) {
      ...
      // (CANAGL) Comment: It is not really ridiculous to send keystrokes to other controls.  
